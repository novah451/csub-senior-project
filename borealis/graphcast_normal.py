...
lat_range = np.arange(-90, 91, 0.25)                               # Latitude range
lon_range = np.arange(0, 360, 0.25)                                # Longitude range
...
# Load the model
with open(r'model/graphcast/params/GraphCast - ERA5 1979-2017 - resolution 0.25 - pressure levels 37 - mesh 2to6 - precipitation input and output.npz', 'rb') as model:
...
def integrateSolarRadiation(data:pd.DataFrame):
    ...
    coords = [[float(lat), float(lon)] for lat in lat_range for lon in lon_range]
    values = []
